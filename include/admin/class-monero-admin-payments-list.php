...
HTML;
    ...
    protected function get_order_link($order_id) {
        try {
            ...
            if (!$order) {
                return '<strong>#' . absint($order_id) . ' (deleted)</strong>';
            }
        } catch (\Exception $e) {
            return '<strong>#' . absint($order_id) . ' (deleted)</strong>';
        }
        ...
        return '<a href="' . esc_url( admin_url( 'admin.php?page=wc-orders&action=edit&id=' . absint( $order->get_id() ) ) ) . '" class="order-view"><strong>#' . esc_attr( $order->get_order_number() ) . ' ' . esc_html( $buyer ) . '</strong></a>';
